...
// D-series label sizes
let LABEL_SIZES = { ...D_SERIES_LABEL_SIZES, ...D_SERIES_ROUND_LABELS };
...
const state = {
  ...
  transport: null,
  renderer: null,
  canPrint: true,   // Set to false if browser doesn't support Web Bluetooth
...
function hideInfoDialog() {
  ...
  safeStorageSet('d30_info_seen', 'true');
}
...
function shouldShowInfoOnLoad() {
  return !safeStorageGet('d30_info_seen');
}
...
function handleExport() {
  ...
  a.href = url;
  a.download = `d30-design-${Date.now()}.json`;
...
function checkCompatibility() {
  ...
  // Allow file:// and localhost — these are treated as secure for local use
  const isLocalFile = location.protocol === 'file:';
  const isLocalhost = location.hostname === 'localhost' || location.hostname === '127.0.0.1';
  const isSecure = window.isSecureContext || isLocalFile || isLocalhost;

  if (!isSecure) {
    warnings.push('HTTPS required for printing - serve this app over HTTPS or open the file directly in your browser');
    canPrint = false;
  }
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-[600px] max-w-full overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-amber-500 to-orange-500 px-5 py-4 text-white relative">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-lg font-bold">Web Bluetooth Required</h3>
              <p class="text-amber-100 text-xs">Printing requires Chrome or Chromium with Web Bluetooth</p>
            </div>
          </div>
        </div>
        ...
        <div class="p-5 space-y-4">
          ...
          <div class="grid grid-cols-1 gap-2">
            <div class="bg-gray-50 rounded-lg p-2 border border-gray-200">
              ...
              <p class="text-sm text-gray-700">D30, D35, D50, D110, Q30, Q30S — Bluetooth BLE</p>
            </div>
          </div>
        ...
        <div class="bg-gray-50 px-5 py-3 border-t border-gray-100 flex items-center justify-between">
          ...
          <p class="text-xs text-gray-400">D30 Label Designer</p>
        </div>
      </div>
    `;
...
function init() {
  ...
  const savedPrintSettings = safeStorageGet('d30_print_settings');
  ...
  console.log('D30 Label Designer initialized');
}
